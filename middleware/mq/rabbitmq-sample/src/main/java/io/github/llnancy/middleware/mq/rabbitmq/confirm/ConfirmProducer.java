package io.github.llnancy.middleware.mq.rabbitmq.confirm;
...
public class ConfirmProducer {
    ...
    public static void main(String[] args) throws Exception {
        // singlePublishConfirmMessage();// 单个发布确认 1000 条消息耗时：1130ms
        // batchPublishConfirmMessage();// 批量发布确认 1000 条消息耗时：110ms
        asyncPublishConfirmMessage();// 异步发布确认 1000 条消息耗时：81ms
    }
        ...
        try (Channel channel = RabbitMqHelper.getChannel()) {
            ...
            channel.addConfirmListener(new ConfirmListener() {
                ...
                public void handleAck(long deliveryTag, boolean multiple) throws IOException {
                    ...
                    System.out.println("确认的消息 deliveryTag：" + deliveryTag);
                }

                @Override
                public void handleNack(long deliveryTag, boolean multiple) throws IOException {
                    String message = confirmMap.get(deliveryTag);
                    System.out.println("未确认的消息的 deliveryTag：" + deliveryTag + "；消息体为：" + message);
                }
            });
