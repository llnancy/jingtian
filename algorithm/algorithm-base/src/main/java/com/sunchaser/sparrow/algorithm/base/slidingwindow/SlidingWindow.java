package com.sunchaser.sparrow.algorithm.base.slidingwindow;
...
public class SlidingWindow {
    ...
    public static void slidingWindow(String s, String t) {
        ...
        while (right < sArr.length) {
            char c = sArr[right];// c是要移入窗口的字符
            right++;// 窗口右移
            // 进行窗口内数据的更新

            // 判断左侧窗口是否需要收缩
            while (true/*window needs shrink*/) {
                char d = sArr[left];// d是要移出窗口的字符
                left++;// 左移窗口
                // 进行窗口内数据更新
            }
        }
    ...
    public static int lengthOfLongestSubstring(String s) {
        ...
        while (right < sArr.length) {
            ...
            // 判断左侧窗口是否需要收缩
            while (window.get(c) > 1) {// 窗口内存在重复字符，需要移动left缩小窗口
                char d = sArr[left];// d是要移出窗口的字符
                left++;// 左移窗口
